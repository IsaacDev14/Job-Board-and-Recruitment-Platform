...
// Assuming your Job type is defined like this in types/job.ts
// It should now align with the backend's job_model and to_dict output
interface Job {
    id: number;
    title: string;
    description: string;
    ...
    salary_range: string; // Added for frontend compatibility
    job_type: string; // Changed to job_type
    type: string; // Added for frontend compatibility (matches existing usage in JSX)
    date_posted: string;
    expires_date?: string;
    recruiter_id: number;
    company_id: number;
    company_name?: string; // These come from the related models
    recruiter_username?: string;
    is_active: boolean; // Added is_active
    image?: string; // Keep this if you intend to add image support
}
...
const Jobs: React.FC<JobsProps> = ({ onNavigate }) => {
    ...
    useEffect(() => {
        const fetchAllJobs = async () => {
            ...
            try {
                ...
                const response = await api.get<Job[]>('/jobs/', { params: { is_active: true } });
    ...
    useEffect(() => {
        if (!isLoading) { // Only filter once initial load is complete
            setIsFiltering(true);
            const handler = setTimeout(() => {
                const normalizedSearch = searchTerm.trim().toLowerCase();

                const filtered: Job[] = allJobs.filter(job => {
                    const companyName = job.company_name ? job.company_name.toLowerCase() : ''; // Use company_name
                            ...
                            >
                                ...
                                <div className="p-5">
                                    ...
                                    <p className="text-gray-700 text-sm mb-1 flex items-center">
                                        ...
                                        {job.company_name || 'Company Not Found'} {/* Use company_name here */}
                                    </p>
                                    ...
                                    <div className="mt-3 flex flex-wrap justify-between items-center text-sm text-gray-500">
                                        <span className="flex items-center mr-3 mb-1">
                                            <FaDollarSign className="text-gray-500 mr-1" /> {job.salary_range} {/* Use salary_range */}
                                        </span>
                                        <span className="flex items-center mb-1">
                                            <FaBriefcase className="text-gray-500 mr-1" /> {job.type || 'Full-time'} {/* Use job.type */}
                                        </span>
                                    </div>
                                    <button
