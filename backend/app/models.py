...
class Company(db.Model):
    ...
    # Relationships
    # 'company' backref from Job model
...
class Job(db.Model):
    ...
    company_id = db.Column(db.Integer, db.ForeignKey('company.id'), nullable=True) # *** CHANGED TO nullable=True ***

    # Relationships
    # 'job' backref from Application model (if you have one)
    applications = db.relationship('Application', backref='job', lazy=True)

    def __repr__(self):
        return f'<Job {self.title}>'

# Assuming you have an Application model, if not, you can remove this
